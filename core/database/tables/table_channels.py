...
class DataBaseChannels(DataBase):
    ...
    def get_channel_tags(self) -> list:
    ...
    def get_all_channels_info(self) -> list:
        self.cursor.execute(
            f"SELECT {KeyChannels.channel_id}, {KeyChannels.channel_tag}, {KeyChannels.channel_name} FROM channels")
        rows = self.cursor.fetchall()
        return rows

    def add_channel_to_db(self, channel_id: str, channel_tag: str, channel_name: str) -> None:
        self.cursor.execute(
    ...
    def del_channel(self, channel_id: str) -> None:
