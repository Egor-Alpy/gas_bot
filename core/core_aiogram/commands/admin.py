...
async def add_channel(message: types.Message, LAN: str):
...
async def del_channel(message: types.Message, LAN: str):
...
async def send_msg_to_all(message: types.Message, LAN: str):
...
async def send_message_to_user_promo(message, chat_id: int):
    try:
        await message.send_copy(chat_id)
    except Exception as e:
        pass  # can't send the message to user {user_id}
